...
test("completes the general FocusProof review loop and refresh recovery", async ({ page }, testInfo) => {
  ...
  await page.getByLabel("Learning domain").selectOption("programming");
  ...
  await page.getByLabel("Learning notes, explanation, code, or error record").fill("EventLog entries are immutable facts. A View is derived and can be rebuilt from replay.");
...
test("keeps Web3 evidence optional and isolated from the general flow", async ({ page }, testInfo) => {
  ...
  await page.getByLabel("What did this operation complete?").fill("Observed a local test transaction without writing proof on-chain.");
