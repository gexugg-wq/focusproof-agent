...
from focusproof.domain.plugins.monad.models import MonadEvidence, MonadVerificationObservation
from focusproof.domain.plugins.monad.rpc_client import MonadRpcClient


class MonadEvidenceVerifier:
    def __init__(self, *, rpc: MonadRpcClient, chain_id: int, contract_address: str,
                 deployment_block: int, increment_selector: str, incremented_topic: str,
                 session_time_tolerance_seconds: int = 300) -> None:
        self._rpc = rpc
    ...
    def verify(self, evidence: MonadEvidence, session_started_at: datetime) -> MonadVerificationObservation:
        wallet = _normalize_address(evidence.wallet_address)
        if wallet is None:
            return _result("rejected", "invalid_wallet_address")
        try:
            if self._rpc.chain_id() != self._chain_id:
                return _result("rejected", "wrong_chain")
            transaction = self._rpc.transaction(evidence.transaction_hash)
            if transaction is None:
                return _result("unavailable", "transaction_unavailable", retryable=True)
            receipt = self._rpc.receipt(evidence.transaction_hash)
            if receipt is None:
            ...
            if block_number < self._deployment_block or not self._rpc.code(self._contract, block_number):
            ...
            if new != previous + 1:
                return _result("rejected", "invalid_increment_transition", block_number=block_number)
            timestamp = self._rpc.block_timestamp(block_number)
        ...
        facts: dict[str, int | str] = {"chain_id": self._chain_id,
            "transaction_hash": evidence.transaction_hash.lower(), "sender": wallet,
            "target": self._contract, "previous_value": previous, "new_value": new}
    ...
    def _find_event(self, logs: object) -> tuple[str, int, int] | None:
        if not isinstance(logs, list):
            return None
        for raw in logs[:64]:
            if not isinstance(raw, dict) or _normalize_address(str(raw.get("address", ""))) != self._contract:
                continue
            topics = raw.get("topics", [])
            if not isinstance(topics, list) or len(topics) != 2 or str(topics[0]).lower() != self._topic:
                continue
            learner = _normalize_address("0x" + str(topics[1])[-40:])
...
def _result(status: str, finding: str, *, retryable: bool = False,
            block_number: int | None = None) -> MonadVerificationObservation:
    return MonadVerificationObservation(status, {}, (finding,), block_number, retryable)  # type: ignore[arg-type]
